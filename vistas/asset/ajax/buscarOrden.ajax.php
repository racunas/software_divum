...
$busquedaUsuario = modeloOrden::mdlBuscarOrdenDeUsuario($idUsuario,$tipo,$busqueda);
...
/*if(!$busquedaUsuario){
	...
	foreach ($busquedaUsuario as $key => $value) {
		...
		if( $value['entregado'] == 4 ){
			...
			$estadoTrabajo = '<p class="bold estadoOrdenes ausente"><i class="fas fa-folder-plus mr-2"></i> Ausente</p>';
...
if(!$busquedaUsuario){
	...
	$respuesta .= '<div class="alert alert-warning mt-2">No se encontraron resultados para <b>'.$busqueda.'</b></div>';

} else {

	/*RESULTADOS PARA DISPOSITIVOS MOVILES*/
	...
	foreach ($busquedaUsuario as $key => $value) {
		...
		if( $value['entregado'] == 4 ){
			...
			$estadoTrabajo = '<p class="bold"><i class="fas fa-folder-plus mr-2"></i> Ausente</p>';
...
//echo json_encode($busquedaUsuario);
